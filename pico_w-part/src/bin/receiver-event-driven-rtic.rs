...
// Run with `DEFMT_LOG=info cargo run --release --bin receiver-event-driven-rtic`
...
mod app {
    use rp_pico::{
        hal::{
            ...
            gpio::{bank0::{Gpio13}, FunctionSioInput, Interrupt, Pin, PullNone},
            sio::Sio,
            watchdog::Watchdog,
        },
        XOSC_CRYSTAL_FREQ,
    ...
    // Useful aliases
    pub type IRPin = Pin<Gpio13, FunctionSioInput, PullNone>;
    pub type IrReceiver =
        Receiver<Nec, IRPin, <Monotonic as rtic_monotonic::Monotonic>::Instant>;
    ...
    fn init(c: init::Context) -> (Shared, Local, init::Monotonics) {
        ...
        // Setup the IR stuff
        let ir_sensor_pin = pins.gpio13.into_floating_input();
